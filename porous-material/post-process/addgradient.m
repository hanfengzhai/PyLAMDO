...
%a ADDGRADIENT   Add a nice looking gradient background to a plot
%
...
%
% P = ADDGRADIENT(...)  returns handle to the patch used t the
...
lim = axis(ax);
xdata = [lim(1) lim(1) lim(2) lim(2)];
ydata = [lim(3) lim(4) lim(4) lim(3)];
cdata(1,1,:) = topcolor;
cdata(4,1,:) = topcolor;%bottomcolor;
cdata(3,1,:) = bottomcolor;
cdata(2,1,:) = bottomcolor;
