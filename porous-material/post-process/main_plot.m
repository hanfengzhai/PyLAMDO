...
D = 0.1:0.1:0.9;
%%
colormap('jet'); % You can choose any other colormap here

num_lines = size(surface_area, 1);
c_map = colormap; % Get the colormap values
line_colors = c_map(round(linspace(1, size(c_map, 1), num_lines)), :);

subplot(2,2,1)
hold on;
for i = 1:num_lines
    plot(N_unit, surface_area(i, :), 'Color', line_colors(i, :), 'LineWidth', 1.5);
end
hold off;texadd;addgradient;colorbar off;box on
subplot(2,2,3)
hold on;
for i = 1:num_lines
    plot(N_unit, vacuum_region(i, :), 'Color', line_colors(i, :), 'LineWidth', 1.5);
end
hold off;texadd;addgradient;colorbar off;box on
num_lines = size(surface_area, 2);
c_map = colormap; % Get the colormap values
line_colors = c_map(round(linspace(1, size(c_map, 1), num_lines)), :);
...
hold on;
for i = 1:num_lines
    plot(D, surface_area(:,i), 'Color', line_colors(i, :), 'LineWidth', 1.5);
end
hold off;texadd;addgradient;colorbar off;box on
subplot(2,2,4)
hold on;
for i = 1:num_lines
    plot(D, vacuum_region(:,i), 'Color', line_colors(i, :), 'LineWidth', 1.5);
end
hold off;texadd;addgradient;colorbar off;box on
%%
subplot(2,2,3)
...
texadd;addgradient;colorbar off
subplot(2,2,2)
plot(D,surface_area(:,1));hold on
plot(D,surface_area(:,2));plot(D,surface_area(:,3));plot(D,surface_area(:,4));plot(D,surface_area(:,5));
plot(D,surface_area(:,6));plot(D,surface_area(:,7));plot(D,surface_area(:,8));plot(D,surface_area(:,9));plot(D,surface_area(:,10));
plot(D,surface_area(:,11));plot(D,surface_area(:,12));plot(D,surface_area(:,13));plot(D,surface_area(:,14));plot(D,surface_area(:,15));
texadd;addgradient;colorbar off
subplot(2,2,4)
...
plot(D,vacuum_region(:,6));plot(D,vacuum_region(:,7));plot(D,vacuum_region(:,8));plot(D,vacuum_region(:,9));plot(D,vacuum_region(:,10));
plot(D,vacuum_region(:,11));plot(D,vacuum_region(:,12));plot(D,vacuum_region(:,13));plot(D,vacuum_region(:,14));plot(D,vacuum_region(:,15));
texadd;addgradient;colorbar off
%% functions
